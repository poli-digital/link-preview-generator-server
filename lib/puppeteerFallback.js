...
// A client-side/JS redirect (common on tracking links like /Link/Open/...?r=)
// navigates the page mid-operation, destroying the JS execution context. These
// errors mean "a navigation is in flight", not a real failure — we wait it out
// and re-read the final DOM instead of giving up.
function isNavigationRace(err) {
	return /execution context was destroyed|because of a navigation|cannot find context|does not belong to the document|most likely because of a navigation/i.test(
		(err && err.message) || ""
	);
}

/**
 * Read page.content(), tolerating in-flight client-side redirects: if the
 * context is destroyed by a navigation, wait for it to settle and retry.
 */
async function readSettledContent(page) {
	for (let attempt = 0; attempt < 3; attempt++) {
		try {
			return await page.content();
		} catch (err) {
			if (isNavigationRace(err) && attempt < 2) {
				await page
					.waitForNavigation({ waitUntil: "domcontentloaded", timeout: TIMEOUT_MS })
					.catch(() => {}); // settle (or time out) then retry the read
				continue;
			}
			throw err;
		}
...
async function renderHtml(url) {
	...
	try {
		...
		try {
			...
			// A redirect firing during load can reject the goto itself; the page is
			// still navigating to the real destination, so read it below anyway.
			// Anything that isn't a navigation race is a genuine failure.
			if (!isNavigationRace(err)) {
				throw err;
			}
		}
		const html = await readSettledContent(page);
